...
params = {}
params["to_convert_directory"] = "~/data/rawHD/rawHD_80input/"
params["output_directory"] = "~/data/rawHD/rawHD_80input-spikes"
params["num_frames"] = 20
...
try:
    os.makedirs(os.path.expanduser(params["output_directory"]))
except:
    pass

print("converting training dataset")
np.save(f"{os.path.expanduser(params['output_directory'])}/training_x_spikes.npy", convert_currents_to_spike_times(params, train_x))
np.save(f"{os.path.expanduser(params['output_directory'])}/training_y_spikes.npy", np.array(train_y, dtype = np.uint8))
shutil.copy(f"{os.path.expanduser(params['to_convert_directory'])}/training_details.csv", f"{os.path.expanduser(params['output_directory'])}/training_details.csv")


print("converting testing dataset")
np.save(f"{os.path.expanduser(params['output_directory'])}/testing_x_spikes.npy", convert_currents_to_spike_times(params, test_x))
np.save(f"{os.path.expanduser(params['output_directory'])}/testing_y_spikes.npy", np.array(test_y, dtype = np.uint8))
shutil.copy(f"{os.path.expanduser(params['to_convert_directory'])}/testing_details.csv", f"{os.path.expanduser(params['output_directory'])}/testing_details.csv")
