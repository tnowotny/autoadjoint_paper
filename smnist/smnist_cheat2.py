...
from ml_genn.utils.data import (calc_latest_spike_time, calc_max_spikes,
                                preprocess_spikes)
from utils import smnist_encode
...
times, ids = smnist_encode(images, p["DT"], levels=p["NUM_INPUT"], num_examples=p["NUM_EXAMPLES"])

print(f"len(times): {len(times)}")
for i in range(len(times)):
    if train_labels[i] in p["LABEL"]:
        #print(images[i])
        #plt.figure()
        #plt.imshow(images[i])
        #print(events.shape)
        ...
        max_spikes = max(max_spikes, len(times[i]))
        latest_spike_time = max(latest_spike_time, np.amax(times[i]))
        spikes.append(preprocess_spikes(times[i],ids[i],p["NUM_INPUT"]))
...
for i in range(10):
    for j in range(10):
        id = i*10+j
        ax2[i][j].scatter(times[the_img[id]],ids[the_img[id]],s=1)
plt.show()
